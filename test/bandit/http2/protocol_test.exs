defmodule HTTP2ProtocolTest do
  ...
  describe "RST_STREAM frames" do
    ...
    test "rejects connections that exceed RST_STREAM rate limit", context do
      context = https_server(context, http_2_options: [max_reset_stream_rate: {3, 1000}])
      socket = SimpleH2Client.setup_connection(context)
      port = context[:port]
      ...
      assert_receive {:log, %{level: :error, msg: {:string, msg}}}, 500
      assert msg =~ "Stream resets rate exceeded"
    end

    @tag :capture_log
    test "allows RST_STREAM frames within rate limit", context do
      context = https_server(context, http_2_options: [max_reset_stream_rate: {5, 1000}])
      socket = SimpleH2Client.setup_connection(context)
      port = context[:port]

      # Create streams first by sending headers, then send RST_STREAM frames (within limit)
      for stream_id <- [1, 3, 5] do
        SimpleH2Client.send_simple_headers(socket, stream_id, :get, "/slow_body_response", port)
        SimpleH2Client.send_rst_stream(socket, stream_id, 8)
      end
    ...
    test "disables rate limiting when max_reset_stream_rate is nil", context do
      context = https_server(context, http_2_options: [max_reset_stream_rate: nil])
      socket = SimpleH2Client.setup_connection(context)
      port = context[:port]

      # Create streams first by sending headers, then send many RST_STREAM frames
      for stream_id <- 1..20//2 do
        SimpleH2Client.send_simple_headers(socket, stream_id, :get, "/slow_body_response", port)
        SimpleH2Client.send_rst_stream(socket, stream_id, 8)
      end
